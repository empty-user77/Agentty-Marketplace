...
| `logo` | optional `https://` image drawn instead of the icon once Agentty has fetched it. Up to 400 characters, a plain host (no credentials), and the file has to answer as PNG, JPEG, GIF or WebP, really be one, and stay under 512 KB — an SVG is never kept, so send a raster image. Otherwise the icon is kept. Only installed plugins fetch it; the listing here never does |
